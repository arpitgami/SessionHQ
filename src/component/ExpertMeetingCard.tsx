...
export default function ExpertMeetingCard({ meeting }: { meeting: Meeting }) {
  ...
  useEffect(() => {
    const interval = setInterval(() => {
      ...
      if (diffMin <= 10 && diffMin > 0) {
        setCanJoin(true);
        setRemainingTime(
          `Starts in ${hours > 0 ? `${hours} hr ` : ""}${minutes} min`
        );
      } else if (diffMin <= 0) {
        setCanJoin(true);
  ...
  const [userSessionData, setUserSessionData] = useState<SessionDetails | null>(
    null
  );
    ...
    (async function getData() {
      try {
        const res = await fetch(
          `/api/user/saveuserdata?userId=${meeting.userID}`
        );
        const data = await res.json();
        setUserData(data.userData);
        const sessionRes = await fetch(
          `/api/user/saveUserSessionData?userId=${meeting.userID}`
        );
    ...
    <div className="flex bg-base-200 shadow-md rounded-2xl p-4 my-4 gap-4 w-full max-w-4xl mx-auto">
      ...
      <div className="flex flex-col justify-between flex-1">
        <div>
          ...
          <p className="text-base-content text-opacity-70">{userData.email}</p>
          <p className="text-sm mt-1 text-base-content text-opacity-60">
          ...
          <div className="mt-1 space-y-0.5 text-sm">
            <div className="flex flex-wrap gap-4">
              ...
              <p>
                ...
                {userSessionData.industry}
              </p>
              <p>
                <span className="font-medium">Stage:</span>{" "}
                {userSessionData.stage}
              </p>
            </div>
            <p>
              <span className="font-medium">About Startup:</span>{" "}
              {userSessionData.aboutStartup}
            </p>
            <p>
              <span className="font-medium">Help With:</span>{" "}
              {userSessionData.helpWith}
            </p>
            <p>
              <span className="font-medium">Reason to Talk:</span>{" "}
              {userSessionData.reasonToTalk}
            </p>
          </div>
        </div>
        ...
        <button
          ...
          className={`btn mt-4 px-4 py-2 rounded-lg ${
            canJoin ? "btn-primary" : "btn-disabled"
          }`}
        >
          Join Now
        </button>
