...
export default function ExpertApplicationForm() {
    ...
    const handleSubmit = async (e) => {
        ...
        // Basic validation
        if (!formData.fullName || !formData.email || !formData.linkedin || !formData.headline || !formData.expertise || !formData.experience) {
            alert("Please fill out all required fields in Step 1.");
            return;
        }

        if (!formData.bio || !formData.hourlyRate || !formData.password || !formData.confirmPassword) {
    ...
    return (
        ...
        loadFrom ? (<div className="flex flex-col md:flex-row gap-8 max-w-6xl mx-auto p-6">
            <form onSubmit={handleSubmit} className="flex-1 space-y-6">
                ...
                <div className="flex justify-between mt-6">
                    ...
                    {step < totalSteps && <button type="button" className="btn btn-primary" onClick={nextStep}>Next</button>}
            ...
            <div className="w-full md:w-60">
                <ul className="steps steps-vertical">
                    ...
                    <li className={`step ${step >= 2 ? 'step-primary' : ''}`}>About You</li>
                    <li className={`step ${step >= 3 ? 'step-primary' : ''}`}>Account Setup</li>
                </ul>
            </div>
        </div>)
            :
            ...
            <section className="min-h-[90vh] flex items-center justify-center px-6 py-12">
                <div className="text-center max-w-4xl">
                    <h1 className="text-5xl font-bold mb-6">Join a network of Vetted Experts!!</h1>
                    <p className="text-lg mb-8">Helping the next generation of founders.</p>
                    <button className="btn btn-primary btn-lg" onClick={() => setLoadForm(true)}>Join Now</button>
                </div>
            </section>
    );
}
